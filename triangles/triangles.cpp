...
bool TriangleMesh::intersection_tri(const TriangleIndices& triangle, const Ray& d, Vector& P, Vector& N, double &t)
{
	Vector i = vertices[triangle.vtxi];
	Vector j = vertices[triangle.vtxj];
	Vector k = vertices[triangle.vtxk];
	N = cross(j-i,k-i).getNormalized();
	return True

}

bool TriangleMesh::intersect_BBox(const Ray& d, const BoundingBox& box)
{
	double t_1_x = (box.m[0] - d.origin[0]) / d.direction[0];
	double t_2_x = (box.M[0] - d.origin[0]) / d.direction[0];
	double t_min_x = std::min(t_1_x, t_2_x);
	double t_max_x = std::max(t_1_x, t_2_x);

	double t_1_y = (box.m[1] - d.origin[1]) / d.direction[1];
	double t_2_y = (box.M[1] - d.origin[1]) / d.direction[1];
	double t_min_y = std::min(t_1_y, t_2_y);
	double t_max_y = std::max(t_1_y, t_2_y);

	double t_1_z = (box.m[2] - d.origin[2]) / d.direction[2];
	double t_2_z = (box.M[2] - d.origin[2]) / d.direction[2];
